...
-----------------------------------
require('modules/HXI/scripts/dynamis/zone_mechanics/beaucedine_mobs')
require('modules/HXI/scripts/dynamis/zone_mechanics/buburimu_mobs')
require('modules/HXI/scripts/dynamis/zone_mechanics/qufim_mobs')
require('modules/HXI/scripts/dynamis/zone_mechanics/tavnazia_mobs')
require('modules/HXI/scripts/dynamis/zone_mechanics/valkurm_mobs')
require('modules/HXI/scripts/dynamis/zone_mechanics/xarcabard_mobs')

require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_bastok_mobs')
require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_beaucedine_mobs')
require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_buburimu_mobs')
require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_jeuno_mobs')
require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_qufim_mobs')
require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_tavnazia_mobs')
require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_san_d_oria_mobs')
require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_valkurm_mobs')
require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_windurst_mobs')
require('modules/HXI/scripts/dynamis/mob_spawns/dynamis_xarcabard_mobs')
-----------------------------------

local m = Module:new('dynamis_spawning')
...
xi.dynamis.normalDynamicSpawn = function(oMob, oMobIndex, target)
    ...
    if zone == nil then
        print('[DEBUG] Zone is nil ending spawn cycle')
        return
    end
    ...
    for job, number in pairs(xi.dynamis.mobList[mobZoneID][oMobIndex].mobchildren) do
        ...
        if oMob:getFamily() == 4 then
            if
                xi.dynamis.mobList[mobZoneID][oMobIndex].info[6] == 2 or
                xi.dynamis.mobList[mobZoneID][oMobIndex].info[6] == 3
            then
        ...
        while (indexJob <= indexEndJob) and (nameObj ~= nil) do
            local mobArg    = zone:insertDynamicEntity({
                ...
                onMobSpawn = function(mobArg)
                    ...
                    then
                        mobArg:addImmunity(xi.immunity.LIGHT_SLEEP)
                        mobArg:addImmunity(xi.immunity.DARK_SLEEP)
                    end
                ...
                onMobEngage = function(mobArg, mobTarget)
                    ...
                    xi.dynamis.mobOnEngage(mobArg, mobTarget)
                end,
...
xi.dynamis.nonStandardDynamicSpawn = function(mobIndex, oMob, forceLink, zoneID, target, oMobIndex)
    ...
    local nonStandardLookup =
    {
        ...
        ['Statue'] =
        {
            ['Vanguard Eye']         = { 'Vanguard Eye' ,      163, 134, 1100,  4 }, -- Vanguard Eye (VEye)
            ['Prototype Eye']        = { 'Prototype Eye' ,     61,  42,  1100,  4 }, -- Prototype Eye (PEye)
            ['Goblin Statue']        = { 'Goblin Statue' ,     158, 134, 1,    92 }, -- Goblin Statue (GStat)
            ['Goblin Replica']       = { 'Goblin Replica' ,    157, 134, 1,    92 }, -- Goblin Statue (GRStat)
            ['Statue Prototype']     = { 'Stat. Prototype' ,   36,  42,  1,    92 }, -- Goblin Statue (GPStat)
            ['Serjeant Tombstone']   = { 'Serj. Tombstone' ,   89,  134, 1100, 93 }, -- Orc Statue (OStat)
            ['Warchief Tombstone']   = { 'War. Tombstone' ,    90,  134, 1100, 93 }, -- Orc Statue (OWStat)
            ['Tombstone Prototype']  = { 'Tomb. Prototype' ,   20,  42,  1100, 93 }, -- Orc Statue (TPStat)
            ['Adamantking Effigy']   = { 'Adamantking Eff' ,   55,  134, 0,    94 }, -- Quadav Statue (QStat)
            ['Adamantking Image']    = { 'Adamantking Img' ,   56,  134, 0,    94 }, -- Quadav Statue (QIStat)
            ['Effigy Prototype']     = { 'Eff. Prototype' ,    9,   42,  0,    94 }, -- Quadav Statue (QPStat)
            ['Avatar Idol']          = { 'Avatar Idol' ,       124, 134, 1100, 95 }, -- Yagudo Statue (YStat)
            ['Manifest Icon']        = { 'Manifest Icon' ,     68,  39,  1100, 95 }, -- Yagudo Statue (YMStat)
            ['Avatar Icon']          = { 'Avatar Icon' ,       123, 134, 1100, 95 }, -- Yagudo Statue (AIStat)
            ['Icon Prototype']       = { 'Icon Prototype' ,    32,  42,  1100, 95 }, -- Yagudo Statue (YPStat)
        },
        ['Nightmare'] =
        {
            -- Buburimu
            ['Nightmare Crab']       = { 'N. Crab' ,           93,  40,  0,  77             }, -- NCra
            ['Nightmare Dhalmel']    = { 'N. Dhalmel' ,        94,  40,  0,  80             }, -- NDha
            ['Nightmare Urganite']   = { 'N. Urganite' ,       95,  40,  0,  251            }, -- NUrg
            ['Nightmare Bunny']      = { 'N. Bunny' ,          97,  40,  0,  206            }, -- NBun
            ['Nightmare Scorpion']   = { 'N. Scorpion' ,       96,  40,  0,  217            }, -- NSco
            ['Nightmare Cockatrice'] = { 'N. Cockatrice' ,     19,  174, 0,  70             }, -- NCoc
            ['Nightmare Crawler']    = { 'N. Crawler' ,        99,  40,  0,  79             }, -- Ncra
            ['Nightmare Eft']        = { 'N. Eft' ,            101, 40,  0,  98             }, -- NEft
            ['Nightmare Mandragora'] = { 'N. Mandragora' ,     98,  40,  0,  178            }, -- NMan
            ['Nightmare Raven']      = { 'N. Raven' ,          100, 40,  0,  55             }, -- NRav
            -- Tavnazia
            ['Nightmare Cluster']    = { 'N. Cluster' ,        40,  42,  0,  5002, nil, 135 }, -- NClu
            ['Nightmare Hornet']     = { 'N. Hornet' ,         10,  42,  0,  5001, nil, 135 }, -- NHor
            ['Nightmare Leech']      = { 'N. Leech' ,          41,  42,  0,  5005, nil, 135 }, -- NLee
            ['Nightmare Makara']     = { 'N. Makara' ,         34,  42,  0,  5004, nil, 135 }, -- NMak
            ['Nightmare Taurus']     = { 'N. Taurus' ,         33,  42,  0,  5006           }, -- NTau
            ['Nightmare Bugard']     = { 'N. Bugard' ,         6,   42,  0,  5003           }, -- NBug
            -- Valk
            ['Nightmare Hippogryph'] = { 'N. Hippogryph' ,     2,   39,  0,  141            }, -- NHip
            ['Nightmare Manticore']  = { 'N. Manticore' ,      3,   39,  0,  179            }, -- NMat
            ['Nightmare Sabotender'] = { 'N. Sabotender' ,     11,  39,  0,  212            }, -- NSab
            ['Nightmare Sheep']      = { 'N. Sheep' ,          13,  39,  0,  226            }, -- NShe
            ['Nightmare Fly']        = { 'N. Fly' ,            4,   39,  0,  113            }, -- NFly
            -- Qufim
            ['Nightmare Gaylas']     = { 'N. Gaylas' ,         80,  41,  0,  47             }, -- NGay
            ['Nightmare Kraken']     = { 'N. Kraken' ,         75,  41,  0,  218            }, -- NKra
            ['Nightmare Raptor']     = { 'N. Raptor' ,         84,  41,  0,  210            }, -- NRap
            ['Nightmare Roc']        = { 'N. Roc' ,            83,  41,  0,  125            }, -- NRoc
            ['Nightmare Snoll']      = { 'N. Snoll' ,          86,  41,  0,  232            }, -- NSno
            ['Nightmare Diremite']   = { 'N. Diremite' ,       82,  41,  0,  81             }, -- NDir
            ['Nightmare Stirge']     = { 'N. Stirge' ,         78,  41,  0,  46             }, -- NSti
            ['Nightmare Tiger']      = { 'N. Tiger' ,          81,  41,  0,  242            }, -- NTig
            ['Nightmare Weapon']     = { 'N. Weapon' ,         77,  41,  0,  110            }, -- NWea
        },
        ['Elemental'] =
        {
            ['Fire Elemental']       = { 'Fire Ele.' ,         14,  38,   17,  0 }, -- FEle
            ['Water Elemental']      = { 'Water Ele.' ,        17,  38,   15,  0 }, -- WEle
            ['Thunder Elemental']    = { 'Thunder Ele.' ,      18,  38,   16,  0 }, -- TEle
            ['Earth Elemental']      = { 'Earth Ele.' ,        13,  38,   13,  0 }, -- EEle
            ['Air Elemental']        = { 'Air Ele.' ,          11,  38,   12,  0 }, -- AEle
            ['Ice Elemental']        = { 'Ice Ele.' ,          15,  38,   14,  0 }, -- IEle
            ['Light Elemental']      = { 'Light Ele.' ,        16,  38,   19,  0 }, -- LEle
            ['Dark Elemental']       = { 'Dark Ele.' ,         12,  38,   18,  0 }, -- DEle
        },
        ['Beastmen'] =
        {
            -- Bastok Time Extentions
            ['Vanguard Vindicator']  = { 'V. Vindicator' ,     19,  134,  0,    337 }, -- QWAR (Bastok)
            ['Vanguard Constable']   = { 'V. Constable' ,      29,  134,  1,    337 }, -- QWHM (Bastok)
            ['Vanguard Militant']    = { 'V. Militant' ,       25,  134,  0,    337 }, -- QMNK (Bastok)
        },
        ['Other'] =
        {
            ['Vanguard Dragon']      = { 'V. Dragon' ,         70,  135,  0,    87 },  -- VDra
        },
    }
    ...
    -- Idk what this is but it works so I am not touching it
    ...
    local mobFunctions =
    {
        ['Statue'] =
        {
            ...
            ['onMobDeath'] = { function(mob, player, optParams)
                -- special function to handle blue or green eye dyna mobs
                -- healing on death
                xi.dynamis.blueOrGreenEyeStatueOnDeath(mob)
            end },
        ...
        ['Beastmen'] =
        {
            ...
            ['onMobEngage'] = { function(mob, mobTarget)
                xi.dynamis.mobOnEngage(mob, mobTarget)
            end },
    ...
    local dropLists =
    {
        ...
        {
            ['Vanguard Eye']        = 9000,
            ['Statue Prototype']    = 9001, -- Goblin
            ['Serjeant Tombstone']  = 9002, -- Orc Statues
            ['Tombstone Prototype'] = 9002, -- Orc Statues
            ['Effigy Prototype']    = 9003, -- Quadav Statues
            ['Icon Prototype']      = 9004, -- Yagudo Statues
            ['Vanguard Dragon']     = 2559,
        },
        [xi.zone.DYNAMIS_BEAUCEDINE] =
        {
            ['Vanguard Eye']        = 9005,
            ['Goblin Statue']       = 9006,
            ['Goblin Replica']      = 9006,
            ['Serjeant Tombstone']  = 9007, -- Orc Statues
            ['Adamantking Effigy']  = 9008, -- Quadav Statues
            ['Avatar Icon']         = 9009, -- Yagudo Statues
        },
        [xi.zone.DYNAMIS_VALKURM] =
        {
            ['Goblin Replica']      = 9010,
            ['Serjeant Tombstone']  = 9011, -- Orc Statues
            ['Adamantking Effigy']  = 9012, -- Quadav Statues
            ['Manifest Icon']       = 9013, -- Yagudo Statues

            ['Nightmare Hippogryph'] = 1792,
            ['Nightmare Manticore']  = 1799,
            ['Nightmare Sabotender'] = 1792,
            ['Nightmare Sheep']      = 1794,
            ['Nightmare Fly']        = 1794,
            ['Cirrate Christelle']   = 9020,
        },
        [xi.zone.DYNAMIS_TAVNAZIA] =
        {
            ['Vanguard Eye']        = 9022,
            ['Nightmare Cluster']   = 1786,
            ['Nightmare Hornet']    = 1795,
            ['Nightmare Leech']     = 1796,
            ['Nightmare Makara']    = 1797,
            ['Nightmare Taurus']    = 2854,
            ['Nightmare Bugard']    = 1795,
        },
        [xi.zone.DYNAMIS_BUBURIMU] =
        {
            ['Goblin Replica']       = 9014,
            ['Serjeant Tombstone']   = 9015, -- Orc Statues
            ['Adamantking Effigy']   = 9016, -- Quadav Statues
            ['Manifest Icon']        = 9017, -- Yagudo Statues
            ['Nightmare Crab']       = 1791,
            ['Nightmare Dhalmel']    = 2796,
            ['Nightmare Urganite']   = 1785,
            ['Nightmare Bunny']      = 1789,
            ['Nightmare Scorpion']   = 1787,
            ['Nightmare Cockatrice'] = 1805,
            ['Nightmare Crawler']    = 1798,
            ['Nightmare Eft']        = 2795,
            ['Nightmare Mandragora'] = 1789,
            ['Nightmare Raven']      = 1788,
        },
        [xi.zone.DYNAMIS_QUFIM] =
        {
            ['Goblin Replica']      = 9018,
            ['Serjeant Tombstone']  = 9019, -- Orc Statues
            ['Adamantking Effigy']  = 9020, -- Quadav Statues
            ['Manifest Icon']       = 9021, -- Yagudo Statues
            ['Nightmare Gaylas']    = 1793,
            ['Nightmare Kraken']    = 1793,
            ['Nightmare Raptor']    = 1793,
            ['Nightmare Roc']       = 1793,
            ['Nightmare Snoll']     = 1803,
            ['Nightmare Diremite']  = 1790,
            ['Nightmare Stirge']    = 1804,
            ['Nightmare Tiger']     = 1804,
            ['Nightmare Weapon']    = 1804,
            ['Fire Elemental']      = 0, -- No drops for elementals
            ['Water Elemental']     = 0,
            ['Thunder Elemental']   = 0,
            ['Earth Elemental']     = 0,
            ['Air Elemental']       = 0,
        ...
        [xi.zone.DYNAMIS_SAN_DORIA] =
        {
            ['Serjeant Tombstone'] = 2201,
            ['Warchief Tombstone'] = 9023,
        },
        [xi.zone.DYNAMIS_WINDURST] =
        {
            ['Avatar Idol']   = 9024,
        ...
        [xi.zone.DYNAMIS_BASTOK] =
        {
            ['Adamantking Effigy']  = 20,
            ['Vanguard Vindicator'] = 9025,
            ['Vanguard Constable']  = 9025,
            ['Vanguard Militant']   = 9025,
        },
    }

    local dropList = dropLists[zoneID][mobName] -- Assign new droplist
    local flags = 0
    if
        zoneID == xi.zone.DYNAMIS_TAVNAZIA and
        oMob == nil
    then
        if nonStandardLookup[mobMobType][mobName][7] ~= nil then
            flags = nonStandardLookup[mobMobType][mobName][7]
        end
    end
    ...
    local mob = zone:insertDynamicEntity({
        ...
        onMobEngage = function(mob, mobTarget)
            local specFunc = mobFunctions[mobMobType]['onMobEngage'][1]
            specFunc(mob)
        ...
        onMobDeath = function(mob, player, optParams)
            -- if the mob type has own special death function then call that as well
            if mobFunctions[mobMobType]['onMobDeath'] then
                local mobDeathFunc = mobFunctions[mobMobType]['onMobDeath'][1]
                mobDeathFunc(mob)
            end
    ...
    if nonStandardLookup[mobMobType][mobName][4] ~= nil then -- If SpellList ~= nil set SpellList
        mob:setSpellList(nonStandardLookup[mobMobType][mobName][4])
    end

    if nonStandardLookup[mobMobType][mobName][5] ~= nil then -- If SkillList ~= nil set SkillList
        mob:setMobMod(xi.mobMod.SKILL_LIST, nonStandardLookup[mobMobType][mobName][5])
    end

    if nonStandardLookup[mobMobType][mobName][6] ~= nil then -- If Floor
        mob:setLocalVar('Floor', nonStandardLookup[mobMobType][mobName][6])
    end

    if xi.dynamis.mobList[zoneID][mobIndex].info[5] ~= nil then
        zone:setLocalVar(string.format('%s', xi.dynamis.mobList[zoneID][mobIndex].info[4]), 0)
        print('Setting local var for mob: ' .. mobName .. ' with index: ' .. mobIndex)
...
xi.dynamis.nmDynamicSpawn = function(mobIndex, oMobIndex, forceLink, zoneID, target, oMob, mainDynaLord)
    ...
    if
        -- Special spawning of Diabolos Shard and Nightmare Antlion
        ((zoneID == xi.zone.DYNAMIS_TAVNAZIA and
        (mobIndex == 252 or
        mobIndex == 3)) or
    ...
    -- Special spawning of Nightmare Worm
    if
        zoneID == xi.zone.DYNAMIS_TAVNAZIA and
        mobIndex == 2
    then
        -- do not use offsets from the player, as this can result in putting the worm in a wall
        xPos = target:getXPos()
        yPos = target:getYPos()
        zPos = target:getZPos()
    end
    ...
    xi.dynamis.nmInfoLookup =
    {
        ...
        -- Orc
        -- Dynamis - Beaucedine (Done)
        ['Cobraclaw Buchzvotch']     = { 'C.Buchzvotch',    65,  134, 493,  0,    5030, 'Beastmen' }, -- CBuc (MNK)
        ['Deathcaller Bidfbid']      = { 'D.Bidfbid',       73,  134, 493,  0,    5030, 'Beastmen' }, -- DBid (SMN)
        ['Drakefeast Wubmfub']       = { 'D.Wubmfub',       88,  134, 493,  0,    5030, 'Beastmen' }, -- DWub (DRG)
        ['Elvaanlopper Grokdok']     = { 'E.Grokdok',       82,  134, 493,  0,    5030, 'Beastmen' }, -- EGro (RNG)
        ['Galkarider Retzpratz']     = { 'G.Retzpratz',     71,  134, 493,  0,    5030, 'Beastmen' }, -- GRet (RNG)
        ['Heavymail Djidzbad']       = { 'H.Djidzbad',      80,  134, 493,  4,    5030, 'Beastmen' }, -- HDji (PLD)
        ['Humegutter Adzjbadj']      = { 'H.Adzjbadj',      60,  134, 493,  0,    5030, 'Beastmen' }, -- HAbz (WAR)
        ['Jeunoraider Gepkzip']      = { 'J.Gepkzip',       63,  134, 493,  7,    5030, 'Beastmen' }, -- JGep (NIN)
        ['Lockbuster Zapdjipp']      = { 'L.Zapdjipp',      79,  134, 493,  0,    5030, 'Beastmen' }, -- LZap (THF)
        ['Mithraslaver Debhabob']    = { 'M.Debhabob',      85,  134, 493,  0,    5030, 'Beastmen' }, -- MDeb (BST)
        ['Skinmask Ugghfogg']        = { 'S.Ugghfogg',      83,  134, 493,  5,    5030, 'Beastmen' }, -- SUgg (DRK)
        ['Spinalsucker Galflmall']   = { 'S.Galflmall',     73,  134, 493,  3,    5030, 'Beastmen' }, -- SGal (RDM)
        ['Taruroaster Biggsjig']     = { 'T.Biggsjig',      84,  134, 493,  1100, 5030, 'Beastmen' }, -- TBig (BLM)
        ['Ultrasonic Zeknajak']      = { 'U.Zeknajak',      87,  134, 493,  6,    5030, 'Beastmen' }, -- UZek (BRD)
        ['Wraithdancer Gidbnod']     = { 'W.Gidbnod',       63,  134, 493,  1,    5030, 'Beastmen' }, -- WGid (WHM)

        -- Dynamis Buburimu (Done)
        ['Elvaansticker Bxafraff']   = { 'E.Bxafraff',      35,  40,  760,  0,    5030, 'Beastmen' }, -- Elva (DRG)
        ['Flamecaller Zoeqdoq']      = { 'F.Zoeqdoq',       34,  40,  760,  1100, 5030, 'Beastmen' }, -- Flam (BLM)
        ['Hamfist Gukhbuk']          = { 'H.Gukhbuk',       46,  40,  760,  0,    5030, 'Beastmen' }, -- Hamf (MNK)
        ['Lyncean Juwgneg']          = { 'L.Juvgneg',       47,  40,  760,  0,    5030, 'Beastmen' }, -- Lync (RNG)

        -- Dynamis - San d'Oria (Done)
        ['Wyrmgnasher Bjakdek']      = { 'W.Bjakdek',       25,  185, 3111, 0,    5030, 'Beastmen' }, -- WyrB (DRG)
        ['Reapertongue Gadgquok']    = { 'R.Gadgquok',      23,  185, 3111, 0,    5030, 'Beastmen' }, -- ReaG (SMN)
        ['Voidstreaker Butchnotch']  = { 'V.Butchnotch',    26,  185, 3111, 7,    5030, 'Beastmen' }, -- VoiB (NIN)
        ['Battlechoir Gitchfotch']   = { 'B.Gitchfotch',    2,   185, 3111, 6,    5030, 'Beastmen' }, -- BatG (BRD)
        ['Soulsender Fugbrag']       = { 'S.Fugbrag',       3,   185, 3111, 6,    5030, 'Beastmen' }, -- SouF (BRD)

        -- Quadav
        -- Dynamis - Beaucedine (Done)
        ['Be\'Zhe Keeprazer']        = { 'B.Keeprazer',     53,  134, 261,  0,    5029, 'Beastmen' }, -- BeZh (SMN)
        ['De\'Bho Pyrohand']         = { 'D.Pyrohand',      43,  134, 261,  1100, 5029, 'Beastmen' }, --DeBh (BLM)
        ['Ga\'Fho Venomtouch']       = { 'G.Venomtouch',    39,  134, 261,  3,    5029, 'Beastmen' }, -- GaFh (WHM)
        ['Go\'Tyo Magenapper']       = { 'G.Magenapper',    44,  134, 261,  0,    5029, 'Beastmen' }, -- GoTy (DRG)
        ['Gu\'Khu Dukesniper']       = { 'G.Dukesniper',    50,  134, 261,  0,    5029, 'Beastmen' }, -- GuKh (RNG)
        ['Gu\'Nha Wallstormer']      = { 'G.Wallstormer',   24,  134, 261,  0,    5029, 'Beastmen' }, -- Guha (WAR)
        ['Ji\'Fhu Infiltrator']      = { 'J.Infiltrator',   37,  134, 261,  0,    5029, 'Beastmen' }, -- JiFh (THF)
        ['Ji\'Khu Towercleaver']     = { 'J.Towercleaver',  51,  134, 261,  0,    5029, 'Beastmen' }, -- JiKh (SAM)
        ['Mi\'Rhe Whisperblade']     = { 'M.Whisperblade',  52,  134, 261,  7,    5029, 'Beastmen' }, -- MiRh (NIN)
        ['Mu\'Gha Legionkiller']     = { 'M.Legionkiller',  47,  134, 261,  4,    5029, 'Beastmen' }, -- MuGh (PLD)
        ['Na\'Hya Floodmaker']       = { 'N.Floodmaker',    28,  134, 261,  3,    5029, 'Beastmen' }, -- NaHy (RDM)
        ['Nu\'Bhi Spiraleye']        = { 'N.Spiraleye',     41,  134, 261,  6,    5029, 'Beastmen' }, -- NuBh (BRD)
        ['So\'Gho Adderhandler']     = { 'S.Adderhandler',  48,  134, 261,  0,    5029, 'Beastmen' }, -- SoGh (BST)
        ['So\'Zho Metalbender']      = { 'S.Metalbender',   46,  134, 261,  0,    5029, 'Beastmen' }, -- SoZh (MNK)
        ['Ta\'Hyu Gallanthunter']    = { 'T.Gallanthunter', 40,  134, 261,  5,    5029, 'Beastmen' }, -- TaHy (DRK)

        -- Dynamis Buburimu (Done)
        ['Gi\'Bhe Flesheater']       = { 'G.Flesheater',    57,  40,  2901, 1,    5029, 'Beastmen' }, -- GiBh (WHM)
        ['Qu\'Pho Bloodspiller']     = { 'Q.Bloodspiller',  56,  40,  2901, 0,    5029, 'Beastmen' }, -- QuPh (WAR)
        ['Te\'Zha Ironclad']         = { 'T.Ironclad',      69,  40,  2901, 4,    5029, 'Beastmen' }, -- TeZh (PLD)
        ['Va\'Rhu Bodysnatcher']     = { 'V.Bodysnatcher',  68,  40,  2901, 0,    5029, 'Beastmen' }, -- VaRh (THF)

        -- Dynamis - Bastok (Done)
        ['Aa\'Nyu Dismantler']       = { 'A.Dismantler',     40,  134, 2907, 5,    5029, 'Beastmen' }, -- AaNy (DRK)
        ['Gu\'Nhi Noondozer']        = { 'G.Noondozer',      53,  134, 2907, 0,    5029, 'Beastmen' }, -- GuNh (SMN)
        ['Be\'Ebo Tortoisedriver']   = { 'B.Tortoisedriver', 48,  134, 2907, 0,    5029, 'Beastmen' }, -- BeEb (BST)
        ['Gi\'Pha Manameister']      = { 'G.Manameister',    43,  134, 2907, 1100, 5029, 'Beastmen' }, -- GiPh (BLM)
        ['Ko\'Dho Cannonball']       = { 'K.Cannonball',     46,  134, 2907, 0,    5029, 'Beastmen' }, -- KoDh (MNK)
        ['Ze\'Vho Fallsplitter']     = { 'Z.Fallsplitter',   40,  134, 2907, 5,    5029, 'Beastmen' }, -- ZeVh (DRK)
        ['Effigy Shield PLD']        = { 'Effigy Shield',    30,  134, 2907, 4,    5029, 'Beastmen' }, -- EPLD (PLD)
        ['Effigy Shield NIN']        = { 'Effigy Shield',    32,  134, 2907, 7,    5029, 'Beastmen' }, -- ENIN (NIN)
        ['Effigy Shield BRD']        = { 'Effigy Shield',    23,  134, 2907, 6,    5029, 'Beastmen' }, -- EBRD (BRD)
        ['Effigy Shield DRK']        = { 'Effigy Shield',    38,  134, 2907, 5,    5029, 'Beastmen' }, -- EDRK (DRK)
        ['Effigy Shield SAM']        = { 'Effigy Shield',    31,  134, 2907, 0,    5029, 'Beastmen' }, -- ESAM (SAM)
        ...
        ['Aitvaras']                 = { 'Aitvaras',        105, 40,  230,      0, 5022, 'Buburimu Dragon'   }, -- Aitv
        ['Alklha']                   = { 'Alklha',          104, 40,  230,      0, 5020, 'Buburimu Dragon'   }, -- Alkl
        ['Barong']                   = { 'Barong',          103, 40,  230,      0, 5018, 'Buburimu Dragon'   }, -- Baro
        ['Basilic']                  = { 'Basilic',         105, 40,  230,      0, 5021, 'Buburimu Dragon'   }, -- Basi
        ['Jurik']                    = { 'Jurik',           105, 40,  230,      0, 5017, 'Buburimu Dragon'   }, -- Juri
        ['Koschei']                  = { 'Koschei',         105, 40,  230,      0, 5023, 'Buburimu Dragon'   }, -- Kosc
        ['Stihi']                    = { 'Stihi',           102, 40,  230,      0, 5015, 'Buburimu Dragon'   }, -- Stih
        ['Stollenwurm']              = { 'Stollenwurm',     105, 40,  230,      0, 5024, 'Buburimu Dragon'   }, -- Stol
        ['Tarasca']                  = { 'Tarasca',         105, 40,  230,      0, 5019, 'Buburimu Dragon'   }, -- Tara
        ['Vishap']                   = { 'Vishap',          105, 40,  230,      0, 5016, 'Buburimu Dragon'   }, -- Vish
        ['Apocalyptic Beast']        = { 'Apoc. Beast',     1,   40,  146,      2, 87,   'Apocalyptic Beast' }, -- Apoc

        -- Dynamis - Valkurm (Done)
        ['Dragontrap_1']             = { 'Dragontrap',     200,  39,  2910, 0,    114,  'Dragontrap'         }, -- Drat
        ['Dragontrap_2']             = { 'Dragontrap',     201,  39,  2910, 0,    114,  'Dragontrap_child'   }, -- Drat
        ['Dragontrap_3']             = { 'Dragontrap',     201,  39,  2910, 0,    114,  'Dragontrap_child'   }, -- Drat
        ['Fairy Ring']               = { 'Fairy Ring',      10,  39,  2910, 0,    2009, 'Fairy Ring'         }, -- FaiR
        ['Nant\'ina']                = { 'Nant\'ina',       8,   39,  2910, 0,    2011, 'Nant\'ina'          }, -- Nant
        ['Stcemqestcint']            = { 'Stcemqestcint',   6,   39,  2910, 0,    2012,  'Stcemqestcint'      }, -- Stce
        ...
        ['Suttung']                  = { 'Suttung',         85,  41,  3131, 0,    5025, 'Suttung'            }, -- Sutt
        ['Stringes']                 = { 'Stringes',        79,  41,  3131, 0,    5026, 'Stringes'           }, -- Stri
        ['Scolopendra']              = { 'Scolopendra',     76,  41,  3131, 0,    5027, 'Scolopendra'        }, -- Scol
        ['Antaeus']                  = { 'Antaeus',         1,   41,  112,  0,    5028, 'Antaeus'            }, -- Anta

        -- Dynamis - Tavnazia Non-Beastmen
        ['Nightmare Antlion']        = { 'N. Antlion' ,     64,  42,  0,    0,    5000, 'Nightmare Antlion' }, -- NAnt
        ['Nightmare Worm']           = { 'N. Worm' ,        7,   42,  1807, 1103, 5007, 'Nightmare Worm'    }, -- NWor
        ['Umbral Diabolos']          = { 'U. Diabolos',     4,   42,  0,    nil,  nil,  'Umbral Diabolos'   }, -- Umb
        ['Diabolos Club']            = { 'D. Club',         4,   42,  0,    108,  5009, 'Diabolos Club'     }, -- DiaC
        ['Diabolos Diamond']         = { 'D. Diamond',      3,   42,  0,    107,  5008, 'Diabolos Diamond'  }, -- DiaD
        ['Diabolos Heart']           = { 'D. Heart',        2,   42,  0,    106,  5011, 'Diabolos Heart'    }, -- DiaH
        ['Diabolos Spade']           = { 'D. Spade',        1,   42,  0,    nil,  5010, 'Diabolos Spade'    }, -- DiaS
        ['Diabolos Shard']           = { 'D. Shard',        5,   42,  0,    nil,  5012, 'Diabolos Shard'    }, -- DiaSh
    ...
    xi.dynamis.nmFunctions =
    {
        ...
        ['Apocalyptic Beast'] =
        {
            ...
            ['onMobMagicPrepare'] = { function(mob, mobTarget, spellId)
                return xi.dynamis.apocMagicPrepare(mob, mobTarget, spellId)
            end },

            ['onMobWeaponSkillPrepare'] = { function(mob, mobTarget)
                return xi.dynamis.apocWeaponSkillPrepare(mob, mobTarget)
            end },
        ...
        ['Cirrate Christelle'] =
        {
            ...
            ['onMobWeaponSkillPrepare'] = { function(mob)
                return xi.dynamis.onWeaponskillPrepCirrate(mob)
            end },
        ...
        ['Nant\'ina'] =
        {
            ...
            ['onMobWeaponSkillPrepare'] = { function(mob)
                return xi.dynamis.onWeaponskillPrepNantina(mob)
            end },
        ...
        {
            ['onMobSpawn'] = { function(mob)
                xi.dynamis.onSpawnDragontrap(mob)
            end },
        ...
        {
            ['onMobSpawn'] = { function(mob)
                xi.dynamis.onSpawnDragontrapChild(mob)
            end },
        ...
        {
            ['onMobSpawn'] = { function(mob)
                xi.dynamis.onSpawnStcemqestcint(mob)
            end },
        ...
        {
            ['onMobSpawn'] = { function(mob)
                xi.dynamis.onSpawnBubuDragon(mob)
            end },
        ...
        ['Nightmare Worm'] =
        {
            ...
            ['onMobEngage'] = { function(mob, mobTarget)
                xi.dynamis.onMobEngageNightmareWorm(mob, mobTarget)
            end },
        ...
        ['Umbral Diabolos'] =
        {
            ...
            ['onMobEngage'] = { function(mob, mobTarget)
                xi.dynamis.onMobEngageUmbralDiabolos(mob, mobTarget)
            end },
        ...
        ['Diabolos Club'] =
        {
            ...
            ['onMobEngage'] = { function(mob, mobTarget)
                xi.dynamis.onMobEngageDiabolos(mob, mobTarget)
            end },
        ...
        {
            ['onMobSpawn'] = { function(mob)
                xi.dynamis.onSpawnScolopendra(mob)
            end },
        ...
        {
            ['onMobSpawn'] = { function(mob)
                xi.dynamis.onSpawnStringes(mob)
            end },
    ...
    local mob = zone:insertDynamicEntity({
        ...
        onMobEngage = function(mob, mobTarget)
            local specFunc = xi.dynamis.nmFunctions[xi.dynamis.nmInfoLookup[mobName][7]]['onMobEngage'][1]
            specFunc(mob)
            if mobTarget:isPet() then
                local petOwner = mobTarget:getMaster()
                mob:updateEnmity(petOwner)
...
xi.dynamis.spawnDynamicPet = function(target, oMob, mobJob)
    ...
    local petList =
    {
        ...
        [xi.job.SMN] =
        {
            ...
            [334] = -- Orc Family
            {
                -- { Name, groupId, groupZoneId, Droplist, SpellList, SkillList }
                [false] = { 'V. Avatar' , 36, 134, 0, 0, 34 }, -- Vanguard's Avatar (VAva)
                [true] = -- Orc NMD
                {
    ...
    local petFunctions =
    {
        [xi.job.SMN] =
        {
            ['Apocalyptic Beast'] =
            {
                ['onMobInitialize'] = { function(mob, mobTarget)
                end },
    ...
    local mob = zone:insertDynamicEntity({
        ...
        onMobInitialize = petFunctions[mobJob][functionLookup]['onMobInitialize'][1],
        onMobFight      = petFunctions[mobJob][functionLookup]['onMobFight'][1],
        onMobRoam       = petFunctions[mobJob][functionLookup]['onMobRoam'][1],
        onMobEngage     = petFunctions[mobJob][functionLookup]['onMobEngage'][1],
        onMobDeath      = function(mob, player, optParams)
...
xi.dynamis.generatePath = function(mob, mobIndex) -- Handle pathing.
    local zoneID = mob:getZoneID()
    if
        xi.dynamis.mobList[zoneID][mobIndex] ~= nil and
        xi.dynamis.mobList[zoneID][mobIndex].patrolPath ~= nil
    then
...
xi.dynamis.setJob2Hr = function(mob)
    local job2hr =
    {
    ...
    then
        -- Do nothing
    elseif job == job2hr[job] then
        xi.mix.jobSpecial.config(mob, {
            specials =
            {
                { id = job2hr[job], hpp = math.random(20, 70) }
            }
        })
    end
end
...
xi.dynamis.setMobStats = function(mob)
    if mob ~= nil then
        ...
        if mob:getFamily() == 359 then -- If Hydra
            mob:setMobLevel(math.random(82, 83))
        ...
        xi.dynamis.generalStats(mob)
        xi.dynamis.setJob2Hr(mob)
    end
end

xi.dynamis.setNightmareStats = function(mob)
    if mob then
        mob:setMobLevel(math.random(80, 81))
...
xi.dynamis.generalStats = function(mob)
...
xi.dynamis.teleport = function(mob, hideDuration)
    ...
    mob:timer(hideDuration, function(mobT)
        mobT:hideName(false)
        mobT:setUntargetable(false)
        mobT:setAutoAttackEnabled(true)
        mobT:setMagicCastingEnabled(true)
        mobT:setMobAbilityEnabled(true)
    ...
    if mob:getLocalVar('dynamisMobOnDeathTriggered') == 1 then
        ...
        for waveNumber, wave in pairs(xi.dynamis.mobList[zoneID].waveDefeatRequirements) do
            ...
            for waveNum, var in pairs(wave) do
                ...
                totalMobsNeededForWave = totalMobsNeededForWave + 1
            end

            if
                check == totalMobsNeededForWave and
                waveSpawned ~= 1 and
                waveNumber ~= 1
            then
        ...
        -- Handle Dynamis time extentions
...
m:addOverride('xi.dynamis.megaBossOnDeath', function(mob, player, optParams)
    if mob:getLocalVar('dynamisMobOnDeathTriggered') == 1 then
        ...
        local zoneID = mob:getZoneID()
        print('[DYNADEBUG] megaBossOnDeath zoneID: ' .. tostring(zoneID))
        print('[DYNADEBUG] megaBossOnDeath mobID: ' .. tostring(mob:getID()))
        local mobIndex = mob:getZone():getLocalVar(string.format('MobIndex_%s', mob:getID()))
        print('[DYNADEBUG] megaBossOnDeath mobIndex: ' .. tostring(mobIndex))
        local mobVar = xi.dynamis.mobList[mob:getZoneID()][mobIndex].info[5]
        print('[DYNADEBUG] megaBossOnDeath mobVar: ' .. tostring(mobVar))
...
-----------------------------------
xi.dynamis.blueOrGreenEyeStatueOnDeath = function(mob, player)
    if
        (mob:getLocalVar('eyeColor') == xi.dynamis.eye.GREEN or
        mob:getLocalVar('eyeColor') == xi.dynamis.eye.BLUE) and
        -- only heal players once (not for each player)
        mob:getLocalVar('healedPlayers') == 0
    then
        for _, member in pairs(mob:getZone():getPlayers()) do
            if
                member ~= nil and
                member:isAlive() and
                mob:checkDistance(member) <= 25.0
            then
                if mob:getLocalVar('eyeColor') == xi.dynamis.eye.BLUE then
                    local hp = member:getMaxHP() - member:getHP()
                    member:addHP(hp)
                    member:wakeUp()
                    -- this only sends the message to the specific player (thus others in range will not know)
                    member:messageBasic(xi.msg.basic.AOE_REGAIN_HP, 0, hp)
                    -- TODO: improve injectActionPacket to allow AoE type messages so we can use it here
                    -- inject action packet so players see the animation and the message
                    -- mob:injectActionPacket(member:getID(), 11, 772, 0x10, 0x08, xi.msg.basic.AOE_REGAIN_HP, 1124, hp)
                else
                    local mp = member:getMaxMP() - member:getMP()
                    member:addMP(mp)
                    -- this only sends the message to the specific player (thus others in range will not know)
                    member:messageBasic(xi.msg.basic.AOE_REGAIN_MP, 0, mp)
                    -- TODO: improve injectActionPacket to allow AoE type messages so we can use it here
                    -- inject action packet so players see the animation and the message
                    -- mob:injectActionPacket(member:getID(), 11, 773, 0x10, 0x08, xi.msg.basic.AOE_REGAIN_MP, 1125, mp)
                end
            end
        end

        mob:setLocalVar('healedPlayers', 1)
    end
end
