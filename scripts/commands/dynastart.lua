...
commandObj.onTrigger = function(player, mode)
    ...
    if not mode or mode == '' then
        player:printToPlayer('Usage: !dynastart mode', xi.msg.channel.SYSTEM)
        player:printToPlayer('Mode: 0 = normal, 1 = debug', xi.msg.channel.SYSTEM)
        player:printToPlayer('Player must be inside the starting connecting zone.', xi.msg.channel.SYSTEM)
        return
    end
